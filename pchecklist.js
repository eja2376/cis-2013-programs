...
var passwordCheckOld = function () 
{
   ...
   if (stringPassword.length < 8) //the if statement is to make sure the user's password is at least eight characters long, if not, then they have to type a new password.
   {
		stringOutput = ("Your password is too short. Please enter a new password."); 
	}
   else //once their password is at least 8 characters long, the password will be put through a for loop to make sure it isn't considered a bad password.
   {
      for (var intCount = 0; intCount < arrayBadPasswords.length; intCount++)
         {
             if (stringPassword === arrayBadPasswords[intCount])
             {
                stringOutput = ("The password you have entered is considered a bad password.");
             } 
         }//this for loop goes through every element of the arrayBadPasswords to make sure the user password isn't equal to one of the bad passwords.
      if (stringOutput != ("The password you have entered is considered a bad password."))
            {
               stringOutput = ("The password looks great!");
            } //the if statement is to verify their password is a good password by making sure it wasn't found in the array of bad passwords.
